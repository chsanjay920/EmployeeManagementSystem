...
export class EmployeeRegisterComponent {
  ...
  EmployeeForm = new FormGroup({
    ...
    EmpID:new FormControl(((Math.floor(Math.random() * 10000) + 10000).toString().substring(1)).toString())
  });
  onSubmit()
  {
    if (this.EmployeeForm.status == 'VALID') 
    {
        ...
        debugger;
        this.employeeService.postEmployee(EmployeeDataObject).subscribe((data:any)=>{
          console.log(data)
        })
    }
  }
